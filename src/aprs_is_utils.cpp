...
namespace APRS_IS_Utils {
    ...
    void connect() {
        ...
        if (count == 20) {
            ...
            aprsauth = "user " + Config.callsign + " pass " + Config.aprs_is.passcode + " vers IW5ALZ_LoRa_iGate 1.0 filter " + Config.aprs_is.filter;
            upload(aprsauth);
            delay(200);
        }
    }
    ...
    bool processReceivedLoRaMessage(const String& sender, const String& packet) {
        ...
        if (packet.indexOf("{") > 0) {     // ack?
            ...
            if (Config.beacon.path == "") {
                STATION_Utils::addToOutputPacketBuffer(Config.callsign + ">APZMDM,RFONLY::" + processedSender + ":" + ackMessage);
            } else {
                STATION_Utils::addToOutputPacketBuffer(Config.callsign + ">APZMDM,RFONLY," + Config.beacon.path + "::" + processedSender + ":" + ackMessage);
            }
            ...
            if (Config.aprs_is.messagesToRF && packet.indexOf("::") > 0) {
                ...
                if (Addressee == Config.callsign) {             // its for me!
                    if (AddresseeAndMessage.indexOf("{") > 0) {     // ack?
                        ...
                        String ackPacket = Config.callsign + ">APZMDM,TCPIP,qAC::" + Sender + ":" + ackMessage;
                        #ifdef ESP32_DIY_LoRa_A7670
